...
transformer.save_weights(FILE)

persian_vocab = persian_vectorization.get_vocabulary()
...
def decode_sequence(input_sentence):
    ...
    for i in range(max_decoded_sentence_length):
        tokenized_target_sentence = persian_vectorization([decoded_sentence])[:, :-1]
        predictions = transformer([tokenized_input_sentence, tokenized_target_sentence])

        sampled_token_index = np.argmax(predictions[0, i, :])
        sampled_token = persian_index_lookup[sampled_token_index]
